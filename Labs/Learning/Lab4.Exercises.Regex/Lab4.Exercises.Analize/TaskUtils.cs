...
results and of information. 
   ...
   @param punctuation – punctuation marks to separate words 
   @param vowels – vowels of the alphabet   */
        public static void Process(string fin, string fout, string finfo, char[]
     punctuation, string vowels)
        {
                ...
                using (var writerI = File.CreateText(finfo))
                {
                    ...
                    foreach (string line in lines)
                        if (line.Length > 0)
                        {
                            ...
                            string newLine = line.Substring(0, longestWord.Length);
                            //newLine.Append(wordNoVowels);
