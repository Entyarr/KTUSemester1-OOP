...
namespace Lab2.Exercises.Register
{
    static class InOutUtils
    {
        ...
        public static void PrintDogs(DogsRegister Dogs)
        {
            ...
            for(int i = 0; i < Dogs.DogsCount(); i++)
            {
                Console.WriteLine("| {0,8} | {1,-15} | {2,-15} | {3,-12:yyyy-MM-dd} | {4,-8} |",
                    Dogs.GetDog(i).ID, Dogs.GetDog(i).Name, Dogs.GetDog(i).Breed, Dogs.GetDog(i).BirthDate, Dogs.GetDog(i).Gender);
            }
            Console.WriteLine(new string('-', 74));
        }
        public static void PrintUnvaxxed(List<Dog> Bad)
        {
            ...
            {
                Console.WriteLine("| {0,8} | {1,-15} | {2,-15} | {3,-12:yyyy-MM-dd} | {4,-8} |",
                    Dogs.ID, Dogs.Name, Dogs.Breed, Dogs.BirthDate, Dogs.Gender);
            }
            Console.WriteLine(new string('-', 74));
